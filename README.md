...
**Расположение config.php:**
- Для разработки: `config.php` в корне проекта (текущая директория)
- Для production: рекомендуется размещать `config.php` на уровень выше корня проекта (в родительской директории) для безопасности
- Скрипт автоматически ищет файл в обоих местах

1. Создайте или откройте файл `config.php` (можно скопировать из `config.php.example`)
